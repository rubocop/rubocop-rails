...
module RuboCop
  module Cop
    module Rails
      # This cop enforces the grouping of attribute validations.
      #
      # @example
      #   # bad
      ...
      class ValidationsGrouping < Base
        ...
        def check_for_same_attributes_validations(validation_node, attribute_nodes)
          attribute_nodes.each do |attribute_node|
            ...
            @validations[attribute_name] << validation_node

            next unless (validation_nodes = @validations[attribute_name]).size > 1 && same_options?(validation_nodes)
        ...
        def extra_options(options)
          options.reject { |k, _v| VALIDATION_KEYS.include?(k) }
        end
